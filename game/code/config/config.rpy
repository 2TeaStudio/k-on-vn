init -1 python hide:
    ...
    config.window_title = u"K-On! VN: Tea Is (Not) Over"
...
python early:
    config.save_directory = "K-on VN-1458261791"
...
init python:
    ...
    build.directory_name = "K-on_VN-1.0"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "K-on_VN"
    ...
    build.classify('.editorconfig', None)
